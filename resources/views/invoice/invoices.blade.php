...
<head>
  ...
  <style>
    ...
    .card { background: white; border-radius: 10px; border: 1px solid #e2e8f0; box-shadow: 0 4px 12px rgba(15, 23, 42, 0.05); }
    .pill { border-radius: 999px; padding: 4px 10px; font-weight: 500; font-size: 11px; display: inline-block; }
    .dropzone { border: 2px dashed #cbd5e1; border-radius: 6px; padding: 10px; text-align: center; cursor: pointer; background: #f8fafc; transition: border-color 0.2s; }
    .dropzone:hover { border-color: #94a3b8; }
    .sig-line { border-top: 1px solid #e2e8f0; margin-top: 8px; padding-top: 4px; text-align: center; font-size: 11px; color: #64748b; }
    .compact-input { border: 1px solid #e2e8f0; border-radius: 6px; padding: 6px 10px; font-size: 13px; width: 100%; transition: border-color 0.2s; }
    .compact-input:focus { outline: none; border-color: #94a3b8; }
    .compact-label { font-size: 12px; font-weight: 500; color: #475569; margin-bottom: 4px; display: block; }
    .tab-btn { padding: 8px 16px; border-radius: 6px; font-size: 13px; font-weight: 500; border: 1px solid #e2e8f0; background: white; transition: all 0.2s; }
    .tab-btn:hover { background: #f1f5f9; }
    .tab-btn.active { background: #0f172a; color: white; border-color: #0f172a; }
    .action-btn { padding: 8px 16px; border-radius: 6px; font-size: 13px; font-weight: 500; transition: all 0.2s; }
    .primary-btn { background: #0f172a; color: white; }
    .primary-btn:hover { background: #1e293b; }
    .secondary-btn { background: #f1f5f9; color: #475569; flex:none }
    .secondary-btn:hover { background: #e2e8f0; }
    .danger-btn { background: #fee2e2; color: #dc2626; }
    ...
    @media print {
      ...
      .no-print, .edit-terms-btn, [class*="no-print"] {
      ...
      .print-description, .print-signature, .print-terms {
    ...
    .modal-center { max-width: 900px; width: calc(100% - 40px); }
    @keyframes fadeIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
    .fade-in { animation: fadeIn 5.1s ease; }
    ...
    .icon-btn { width: 28px; height: 28px; display: inline-flex; align-items: center; justify-content: center; border-radius: 6px; cursor: pointer; transition: all 0.2s; }
    .icon-btn-sm { width: 24px; height: 24px; font-size: 12px; }
    .terms-item { display: flex; gap: 8px; align-items: center; margin-bottom: 4px; }
    .terms-input { flex: 1; border: 1px solid #e2e8f0; border-radius: 4px; padding: 4px 8px; font-size: 12px; }
    .edit-terms-btn { position: absolute; top: 10px; right: 10px; }
  
    /* Special class for PDF generation */
    .pdf-mode .print-only { display: block !important; }
    .pdf-mode .screen-only { display: none !important; }
    .pdf-mode .edit-terms-btn { display: none !important; }
...
<div class="max-w-7xl mx-auto p-5">
  ...
  <header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
    <div class="flex items-center gap-3 hidden md:block">
      {{-- <img id="main-logo" src="{{ asset('/storage/'. $company->logo) }}" class="h-12 object-contain" alt="Logo" /> --}}
      <div>
        ...
        <div class="space-y-5">
          ...
          <div class="card p-4">
            ...
            <div class="space-y-3">
              ...
              <textarea id="client-address" rows="3" class="compact-input" placeholder="Address & GSTIN"></textarea>
            </div>
          </div>
            ...
            <div class="space-y-3">
              <div class="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input id="invoice-number" class="compact-input" value="{{ $invoice_number }}" />
                </div>
                <div>
              ...
              <div>
                ...
                <input id="discount" type="number" min="0" step="0.01" class="compact-input" value="0" />
              </div>
              <div>
                <label class="compact-label">Description for Invoice</label>
                <textarea id="invoice-description" rows="2" class="compact-input" placeholder="Add description that will appear above signature"></textarea>
              </div>
            </div>
          </div>
          ...
          <div class="card p-4">
            ...
            <div id="admin-sig-preview" class="border rounded p-2 h-20 flex items-center justify-center text-xs text-slate-500">
  ...
  <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
    ...
    <div class="flex gap-2 w-full sm:w-auto">
      <button id="add-item" class="action-btn primary-btn text-xs flex-1 sm:flex-none">
        <i class="fas fa-plus mr-1"></i> <span class="hidden xs:inline">Add Item</span><span class="xs:hidden">Add</span>
      </button>
      <button id="clear-items" class="action-btn secondary-btn text-xs flex-1 sm:flex-none hidden">
        <i class="fas fa-trash mr-1"></i> <span class="hidden xs:inline">Clear All</span><span class="xs:hidden">Clear</span>
      </button>
    </div>
  </div>
      ...
      <table class="w-full text-sm">
        ...
        <tbody id="items-body"></tbody>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div id="invoice-preview" class="card p-5 lg:col-span-2 relative" style="min-height: 800px;">
              <!-- Main Content -->
              <div class="flex justify-between items-start">
                <div>
                  {{-- <img id="preview-logo" src="{{ asset('/storage/'. $company->logo) }}" class="h-10 object-contain" alt="Logo" /> --}}
                  <div>
                  ...
                  <div id="preview-company" class="mt-2 text-xs text-slate-600 whitespace-pre-line"></div>
                </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <div id="preview-client" class="text-xs text-slate-600 whitespace-pre-line mt-1"></div>
                </div>
                <div>
                  <div class="font-semibold text-slate-700">Payment Details</div>
                  <div id="preview-bank" class="text-xs text-slate-600 whitespace-pre-line mt-1"></div>
                </div>
              </div>
              ...
              <div class="mt-4 flex justify-end">
                <div class="w-64 text-sm space-y-1">
                  <div class="flex justify-between"><span class="text-slate-600">Subtotal</span><span id="preview-subtotal"></span></div>
                  <div class="flex justify-between"><span id="preview-tax-label" class="text-slate-600">Tax</span><span id="preview-tax"></span></div>
                  <div class="flex justify-between"><span class="text-slate-600">Discount</span><span id="preview-discount"></span></div>
                  <hr class="my-2 border-slate-200" />
                  <div class="flex justify-between font-bold text-lg"><span>Total</span><span id="preview-total"></span></div>
                </div>
              </div>
              ...
              <div class="print-description hidden print:block">
                ...
                <div id="print-description-text" class="text-xs text-slate-600 whitespace-pre-line border border-slate-200 rounded p-3 mt-1"></div>
              </div>
            
              <div class="print-signature hidden print:block" style="float: right; text-align: right; width: 200px; margin-top: 40px;">
                <div class="font-semibold text-xs text-slate-700">Authorized Signature</div>
                <div id="print-sig-block" class="border border-slate-200 rounded p-3 h-24 flex items-center justify-center mt-1">
                  <div id="print-sig-image" class="text-slate-400 text-xs">No signature uploaded</div>
                </div>
              ...
              <div class="print-terms hidden print:block mt-6" style="clear: both;">
                ...
                <div class="text-xs text-slate-500 mt-2 text-center">Thank you for your business!</div>
              </div>
              ...
              <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4 screen-only">
                <div>
                  ...
                  <div id="screen-description" class="text-xs text-slate-600 whitespace-pre-line mt-1 border border-slate-200 rounded p-3 min-h-20"></div>
                </div>
                <div class="signature-right">
                  <div class="font-semibold text-xs text-slate-700">Authorized Signature</div>
                  <div id="sig-block" class="border border-slate-200 rounded p-3 h-28 flex items-center justify-center mt-1">
                    <div id="sig-image" class="text-slate-400 text-xs">No signature uploaded</div>
                  </div>
                  <div class="sig-line">Authorized Signature</div>
                </div>
              </div>
              ...
              <div class="mt-4 screen-only relative" style="clear: both;">
                ...
                <button id="edit-terms-btn" class="edit-terms-btn icon-btn secondary-btn icon-btn-sm no-print">
            ...
            <div class="space-y-4">
              ...
              <div class="card p-4">
                <div class="space-y-2">
                  <div class="flex justify-between text-sm"><span class="text-slate-600">Subtotal</span><span id="side-subtotal">₹0.00</span></div>
                  <div class="flex justify-between text-sm"><span id="side-tax-label" class="text-slate-600">Tax</span><span id="side-tax">₹0.00</span></div>
                  <div class="flex justify-between text-sm"><span class="text-slate-600">Discount</span><span id="side-discount">₹0.00</span></div>
                  <hr class="my-2 border-slate-200" />
                  <div class="flex justify-between font-bold text-lg"><span>Grand Total</span><span id="side-total">₹0.00</span></div>
                </div>
              </div>
              ...
              <div class="card p-4 no-print space-y-2">
                ...
                <button id="btn-pdf" class="action-btn primary-btn w-full" style="background: #2563eb;">
                  <i class="fas fa-file-pdf mr-2"></i> Download PDF
                </button>
                <button id="btn-print" class="action-btn primary-btn w-full hidden" style="background: #059669;">
      ...
      <div class="card p-4">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-4">
          ...
          <div class="flex gap-2 no-print">
            <input id="history-search" class="compact-input w-48" placeholder="Search by client or number" />
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-sm">
            <thead class="bg-slate-50 text-xs text-slate-600">
              <tr>
                <th class="p-2 text-center w-10"><input id="history-check-all" type="checkbox" /></th>
...
async function saveCompanyDetails(e) {
    ...
    try {
        ...
        if (result.success) {
            ...
            alert('Error saving company: ' + (result.message || 'Unknown error'));
        }
    } catch (error) {
        console.error('Error saving company:', error);
...
async function saveInvoiceToStorage() {
    ...
    document.querySelectorAll('#items-body tr').forEach((tr, index) => {
        ...
        if (desc || service) {
            ...
            if (price < 0) {
                alert(`Item ${index + 1}: Price cannot be negative`);
    ...
    if (invalidItems.length > 0) {
        alert(`Please add description or service type for item(s): ${invalidItems.join(', ')}`);
        return;
    }
  
    if (!hasValidItems) {
        alert('Please add at least one valid item with description or service type');
        return;
    }
...
async function deleteInvoice(id, confirmFirst = true) {
    ...
    if (result.success) {
        ...
        alert('Error deleting invoice: ' + result.message);
    }
}
...
async function bulkDeleteInvoices() {
    ...
    if (result.success) {
        ...
        alert('Error deleting invoices: ' + result.message);
    }
}
async function bulkUpdateStatus() {
    ...
    if (result.success) {
        ...
        alert('Error updating invoices: ' + result.message);
    }
}
async function bulkDownloadPDFs() {
    ...
    alert('Multiple PDF download feature coming soon! For now, please download invoices one by one.');
}
async function bulkPrintInvoices() {
    ...
    alert('Multiple invoice print feature coming soon! For now, please print invoices one by one.');
}
async function bulkEmailInvoices() {
    ...
    alert('Multiple invoice email feature coming soon! For now, please email invoices one by one.');
}
async function bulkShareInvoices() {
    ...
    alert('Sharing multiple invoices:\n\n' +
          ids.map(id => `${API_BASE_URL}/invoices/${id}/share-link`).join('\n'));
}
/* ========== TAB SWITCHING ========== */
    ...
    if (shareBtn) {
        shareBtn.addEventListener('click', () => {
            ...
            if (inv.client_name && inv.items.length > 0) {
                ...
                alert('Please create an invoice first before sharing.');
            }
        });
    }
}
    ...
    if (pdfBtn) {
        pdfBtn.addEventListener('click', () => {
            ...
            if (inv.client_name && inv.items.length > 0) {
                ...
                alert('Please create an invoice first before downloading PDF.');
            }
        });
    }
    ...
    if (printBtn) {
        printBtn.addEventListener('click', () => {
            ...
            if (inv.client_name && inv.items.length > 0) {
                ...
                alert('Please create an invoice first before printing.');
            }
        });
    }
    ...
    if (emailBtn) {
        emailBtn.addEventListener('click', () => {
            ...
            if (inv.client_name && inv.items.length > 0) {
                ...
                alert('Please create an invoice first before sending email.');
            }
        });
    }
