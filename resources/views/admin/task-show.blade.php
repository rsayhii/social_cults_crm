    ...
    <header class="bg-white shadow-sm hidden">
        <div class="flex items-center justify-between px-4 py-3">
            ...
            <div class="flex items-center hidden">
                <div class="w-10 h-10 rounded-lg gradient-bg flex items-center justify-center text-white font-bold mr-3">DM</div>
                ...
                <div class="flex flex-col md:flex-row md:items-start justify-between">
                    <div class="flex-1">
                        ...
                        <div class="flex items-center space-x-4 mt-2">
                            ...
                            <span class="priority-badge status-{{ strtolower(str_replace(' ', '-', $task->status)) }}">{{ $task->status }}</span>
                            @if($task->category)
                                <span class="bg-gray-100 text-gray-800 px-2 py-1 rounded-full text-sm">{{ $task->category }}</span>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="flex items-center">
                        ...
                        <div>
                            ...
                            @if($task->assigner->roles->isNotEmpty())
                                <p class="text-xs text-gray-500">Role: {{ $task->assigner->roles->pluck('name')->implode(', ') }}</p>
                            @endif
                        </div>
                    </div>
                </div>
        ...
        <div class="space-y-2">
            @forelse($task->users as $user)
                <div class="flex items-center gap-2">
                    <div class="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 font-medium text-sm flex-shrink-0">
    ...
    @endif
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach($task->attachments as $attachment)
                            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                ...
                                <a href="{{ Storage::url($attachment) }}" target="_blank" class="text-indigo-600 hover:text-indigo-800">
            ...
            <div class="p-6 bg-gray-50">
                <div class="flex space-x-4">
                    <a href="{{ route('tasks.edit', $task->id) }}" class="bg-indigo-600 text-white px-6 py-2 rounded-lg font-medium hover:bg-indigo-700 transition">
                        Edit Task
                    </a>
                    <a href="{{ route('tasks.index') }}" class="bg-gray-300 text-gray-800 px-6 py-2 rounded-lg font-medium hover:bg-gray-400 transition">
                        Back to List
                    </a>
                </div>
            </div>
