...
<head>
    ...
    <title>Edit Task | Digital Marketing CRM</title>
    ...
    <style>
        ...
        .drag-drop-area { border: 2px dashed #cbd5e1; transition: all 0.3s ease; }
        .drag-drop-area.dragover { border-color: #4f46e5; background-color: #f8fafc; }
        .fade-in { animation: fadeIn 0.3s ease-in-out; }
        @keyframes fadeIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
        .modal-overlay { background-color: rgba(0, 0, 0, 0.5); }
        .existing-attachment { @apply bg-blue-50 border border-blue-200 rounded p-3 mb-2 flex justify-between items-center; }
    ...
    <div class="min-h-screen bg-gray-50 p-4">
        <div class="task-modal-container fade-in">
            ...
            <div class="task-modal-body">
                <div class="assigner-card">
                    ...
                    @if($task->assigner)
                        <div class="flex items-center">
                            <div class="w-12 h-12 rounded-full bg-gradient-to-r from-blue-100 to-indigo-100 flex items-center justify-center text-indigo-600 font-semibold text-lg mr-4">
                ...
                <form id="taskForm" enctype="multipart/form-data" method="POST" action="{{ route('tasks.update', $task->id) }}">
                    @csrf
                        ...
                        <div class="form-grid form-grid-1 gap-6">
                            <div>
                                ...
                                <div class="input-with-icon">
                                    ...
                                    <input type="text" id="taskTitle" name="title" value="{{ $task->title }}" class="form-input" placeholder="Enter task title" required>
                                </div>
                            </div>
                                    ...
                                    <div class="flex space-x-3 items-center">
                                        <div class="input-with-icon flex-1">
                                            ...
                                            <select id="taskCategory" name="category" class="form-input form-select cursor-pointer">
                                                ...
                                                <option value="SEO" {{ $task->category == 'SEO' ? 'selected' : '' }}>SEO</option>
                                                <option value="Ads" {{ $task->category == 'Ads' ? 'selected' : '' }}>Ads</option>
                                    ...
                                    <div id="customCategoryContainer" class="hidden">
                                        <div class="space-y-3">
                                            ...
                                            <div class="flex space-x-3">
                                                <button type="button" id="saveCustomCategory" class="px-4 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 flex items-center justify-center flex-1">
                    ...
                    <div class="form-section">
                        ...
                        <div class="input-with-icon">
                            ...
                            <textarea id="taskDescription" name="description" rows="4" class="form-input" placeholder="Describe the task in detail...">{{ $task->description }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="assignment-toggle">
                            <button type="button" class="toggle-btn {{ !$task->assigned_to_team ? 'active' : '' }}" data-type="individual">
                                <i class="fas fa-user"></i>
                                Individual Users
                            </button>
                            <button type="button" class="toggle-btn {{ $task->assigned_to_team ? 'active' : '' }}" data-type="team">
                                <i class="fas fa-users"></i>
                                Whole Team
                            </button>
                        </div>
                        <input type="hidden" name="assigned_type" value="{{ $task->assigned_to_team ? 'team' : 'individual' }}">
                       
                        <div id="individualAssignment" class="mt-4" style="{{ $task->assigned_to_team ? 'display: none;' : '' }}">
                                    ...
                                    <div class="flex flex-wrap gap-2" id="selectedUsersBadges">
                                        @foreach($task->users as $user)
                                            <div class="selected-users-badge" data-user-id="{{ $user->id }}">
                                                ...
                                                <button type="button" class="ml-2 text-indigo-400 hover:text-indigo-600 remove-selected-user" data-user-id="{{ $user->id }}">
                            ...
                            <div class="user-list-container">
                                ...
                                <div class="p-3 grid grid-cols-1 md:grid-cols-2 gap-2" id="userList">
                                    ...
                                    <div class="user-checkbox-card {{ $task->users->contains($user->id) ? 'selected' : '' }}">
                                        <input type="checkbox" name="assigned_users[]" value="{{ $user->id }}" class="user-checkbox hidden" id="user-{{ $user->id }}" {{ $task->users->contains($user->id) ? 'checked' : '' }}>
                                        <div class="flex items-center justify-between">
                                            ...
                                            <div class="check-indicator w-5 h-5 border-2 border-gray-300 rounded flex items-center justify-center transition-all duration-200 {{ $task->users->contains($user->id) ? 'bg-indigo-500 border-indigo-500' : '' }}">
                                                <i class="fas fa-check text-white text-xs {{ $task->users->contains($user->id) ? '' : 'opacity-0' }}"></i>
                                            </div>
                                        </div>
                                    </div>
                                    @endforeach
                        ...
                        <div id="teamAssignment" class="mt-4" style="{{ !$task->assigned_to_team ? 'display: none;' : '' }}">
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-grid form-grid-3 gap-6">
                            <div>
                                ...
                                <div class="space-y-2">
                                    <div class="priority-card {{ $task->priority == 'Low' ? 'active' : '' }}" data-priority="low" id="priority-low">
                                        <div class="flex items-center">
                                    ...
                                    <div class="priority-card {{ $task->priority == 'Medium' ? 'active' : '' }}" data-priority="medium" id="priority-medium">
                                        <div class="flex items-center">
                                    ...
                                    <div class="priority-card {{ $task->priority == 'High' ? 'active' : '' }}" data-priority="high" id="priority-high">
                                        <div class="flex items-center">
                                ...
                                <input type="hidden" id="priority" name="priority" value="{{ $task->priority ?? 'Medium' }}">
                            </div>
                            ...
                            <div>
                                ...
                                <div class="input-with-icon">
                                    ...
                                    <input type="date" id="dueDate" name="due_date" value="{{ $task->due_date ? $task->due_date->format('Y-m-d') : '' }}" class="form-input cursor-pointer">
                                </div>
                            </div>
                            ...
                            @foreach($task->attachments as $index => $path)
                                <div class="existing-attachment">
                                    ...
                                    <div class="flex items-center space-x-4">
                                        @if(Storage::disk('public')->exists($path))
                                            <a href="{{ Storage::url($path) }}" target="_blank" class="text-indigo-600 hover:text-indigo-800" title="View">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <a href="{{ Storage::url($path) }}" download class="text-green-600 hover:text-green-800" title="Download">
                                                <i class="fas fa-download"></i>
                                            </a>
                                        @endif
                                        <label class="flex items-center cursor-pointer">
                                            <input type="checkbox" name="remove_attachments[]" value="{{ $path }}" class="mr-2 h-4 w-4 text-red-600 focus:ring-red-500 border-gray-300 rounded">
