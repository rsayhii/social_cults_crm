...
@section('content')
     <div class="flex-1 overflow-auto ">
                <div class="max-w-7xl mx-auto space-y-6">
                    ...
                    <style>
                        .proposal-card { transition: all 0.3s ease; cursor: pointer; }
                        .proposal-card:hover { transform: translateY(-10px); box-shadow: 0 20px 40px rgba(0,0,0,0.15); border-color: #6366f1 !important; }
                        .edit-icon { position: absolute; top: 10px; right: 10px; opacity: 0; transition: opacity 0.2s; color: #6366f1; cursor: pointer; }
                        .editable-section:hover .edit-icon { opacity: 1; }
                        [contenteditable="true"]:hover, [contenteditable="true"]:focus { outline: 2px dashed #a5b4fc; background-color: #f8fafc; border-radius: 8px; }
                        #uploadModal.hidden,
...
#saveToast.hidden { 
    display: none !important; 
                        ...
                        .template-active { border-color: #6366f1 !important; background-color: #f8f9ff; }
                        .blank-card { border: 3px dashed #c7d2fe; background: linear-gradient(135deg, #eef2ff 0%, #e0e8ff 100%); min-height: 380px; }
                        .section-highlight { position: relative; padding: 20px; margin-bottom: 2rem; border-radius: 12px; transition: all 0.3s; }
                        .section-highlight:hover { background-color: #f8fafc; }
                        .section-highlight .edit-icon { top: 12px; right: 12px; }
                        .template-item { 
                    ...
                    <header class="bg-white shadow-sm border-b border-gray-200">
                        <div class="flex items-center justify-between px-6 py-4">
                            <div class="flex items-center">
                                ...
                                <span class="ml-2 bg-indigo-100 text-indigo-800 text-xs font-medium px-2 py-1 rounded-full">CRM</span>
                            </div>
                        </div>
                    </header>
                            ...
                            <div class="lg:col-span-1">
                                <div class="bg-white rounded-xl shadow-sm p-6 sticky top-6">
                                    <div class="flex items-center justify-between mb-6">
                                        ...
                                        <button id="addTemplateBtn" class="bg-indigo-600 text-white p-2 rounded-lg hover:bg-indigo-700">
                            ...
                            <div class="lg:col-span-3">
                                <div id="proposalCardsView">
                                    ...
                                    <div id="proposalCardsGrid" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                                        ...
                                        <!-- Blank Card -->
                                        <div class="blank-card proposal-card bg-white rounded-2xl shadow-lg overflow-hidden border-2 flex flex-col items-center justify-center text-center p-10">
                                            ...
                                            <button id="openUploadModal" class="bg-indigo-600 text-white px-8 py-4 rounded-xl font-semibold hover:bg-indigo-700 shadow-lg">
                                                Upload File
                                            </button>
                                        </div>
                                    ...
                                    <div class="bg-white rounded-2xl shadow-xl p-10">
                                        <div class="flex justify-between items-center mb-8">
                                            ...
                                            <div class="flex space-x-4">
                                                <button id="saveProposal" class="bg-green-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-green-700 shadow-lg">
                                        ...
                                        <div id="proposalContent" class="border-2 border-dashed border-gray-200 rounded-xl p-12 bg-white min-h-screen">
                                            <!-- Content injected here -->
                                        </div>
                                        <div class="mt-8 flex justify-end space-x-4">
                                            <button id="exportDOC" class="bg-blue-600 text-white px-8 py-4 rounded-xl font-semibold hover:bg-blue-700 shadow-lg">
                                                <i class="fas fa-file-word mr-2"></i> Export DOC
                                            </button>
                                            <button id="exportPDF" class="bg-indigo-600 text-white hidden px-8 py-4 rounded-xl font-semibold hover:bg-indigo-700 shadow-lg">
                    ...
                    <div id="uploadModal" class="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 hidden">
                        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 p-8">
                            ...
                            <div class="border-4 border-dashed border-indigo-300 rounded-2xl p-12 text-center cursor-pointer hover:border-indigo-500 transition-all relative" id="dropZone">
                            ...
                            <div class="mt-8 flex space-x-4">
                                <button id="cancelUpload" class="flex-1 bg-gray-200 text-gray-800 py-3 rounded-lg hover:bg-gray-300">Cancel</button>
                                <button id="processFileBtn" class="flex-1 bg-indigo-600 text-white py-3 rounded-lg hover:bg-indigo-700 hidden">Process File</button>
                            </div>
                        </div>
                    </div>

                    <!-- Quick Edit Panel -->
                    <div id="quickEditPanel" class="fixed right-0 top-0 h-full w-96 bg-white shadow-2xl border-l transform translate-x-full transition-transform duration-300 z-50">
                        <div class="p-8 h-full overflow-y-auto">
                            ...
                            <div class="space-y-6">
                                <div>
                                    ...
                                    <input id="editClientName" type="text" placeholder="Enter client name" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Company Name</label>
                                    <input id="editClientCompany" type="text" placeholder="Enter company name" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                </div>
                                <button id="applyQuickEdits" class="w-full bg-indigo-600 text-white py-4 rounded-lg font-semibold hover:bg-indigo-700">
                                    Apply Changes & Close
                                </button>
                            </div>
                        </div>
                    ...
                    <!-- Add Template Modal -->
                    <div id="addTemplateModal" class="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 hidden">
                        <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full mx-4 p-8">
                            ...
                            <div class="space-y-5">
                                ...
                                <div id="customFields" class="hidden space-y-5">
                                    <input id="customName" type="text" placeholder="Template Name (e.g. SEO Proposal)" class="w-full px-5 py-3 border rounded-lg">
                                    <textarea id="customDesc" rows="3" placeholder="Short description..." class="w-full px-5 py-3 border rounded-lg"></textarea>
                                ...
                                <div class="flex space-x-4">
                                    <button id="saveTemplate" class="flex-1 bg-indigo-600 text-white py-3 rounded-lg hover:bg-indigo-700 font-medium">Save Template</button>
                                    <button id="closeModal" class="flex-1 bg-gray-200 text-gray-800 py-3 rounded-lg hover:bg-gray-300">Cancel</button>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="saveToast" class="fixed bottom-8 right-4 bg-green-600 text-white px-6 py-3 rounded-lg shadow-lg transform translate-y-full transition-transform duration-300 z-50">
                        <div class="flex items-center">
                    ...
                    <script>
                        ...
                        let customTemplates = [
                            { id: 1, name: "Social Media Marketing Proposal", description: "Complete social media strategy, content calendar, and performance tracking", key: "social", icon: "hashtag", color: "indigo" },
                            { id: 2, name: "Website Development Proposal", description: "Custom website design, development, and ongoing maintenance", key: "website", icon: "globe", color: "blue" },
                            { id: 3, name: "Google Ads Proposal", description: "PPC campaign setup, management, and optimization for maximum ROI", key: "ads", icon: "ad", color: "green" },
                            { id: 4, name: "SEO Proposal", description: "Search engine optimization strategy to improve organic rankings", key: "seo", icon: "search", color: "purple" }
                        ];
                        ...
                        document.getElementById('saveTemplate').onclick = () => {
                            ...
                            customTemplates.push({ id: nextId++, name, description, key, icon, color });
                            renderTemplates();
                        ...
                        document.getElementById('saveProposal').onclick = () => {
                            ...
                            saveToast.classList.remove('translate-y-full');
                            setTimeout(() => {
