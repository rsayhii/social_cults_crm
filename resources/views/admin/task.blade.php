...
<head>
    ...
    <title>Task Assignment | Digital Marketing CRM</title>
...
<body class="bg-gray-50 min-h-screen">
    ...
    <div class="bg-white rounded-xl shadow-sm p-2 md:p-6 border border-gray-200">
        <div class="flex items-center">
            <div class="w-12 h-12 rounded-lg bg-gradient-to-r from-yellow-100 to-amber-100 flex items-center justify-center text-yellow-600 mr-4">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="bg-white rounded-xl shadow-sm p-2 md:p-6 border border-gray-200">
        <div class="flex items-center">
            <div class="w-12 h-12 rounded-lg bg-gradient-to-r from-blue-100 to-cyan-100 flex items-center justify-center text-blue-600 mr-4">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="bg-white rounded-xl shadow-sm p-2 md:p-6 border border-gray-200">
        <div class="flex items-center">
            <div class="w-12 h-12 rounded-lg bg-gradient-to-r from-green-100 to-emerald-100 flex items-center justify-center text-green-600 mr-4">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <table class="w-full tasks-table">
                        <thead class="bg-gray-50 hidden md:table-header-group">
                            <tr class="border-b border-gray-200">
                                <th class="sortable-header py-3 px-4 text-left text-sm font-semibold text-gray-700" data-sort="title">
                                    Task Title <span class="sort-icon"></span>
                                </th>
                                <th class="py-3 px-4 text-left text-sm font-semibold text-gray-700">Assigned To</th>
                                <th class="sortable-header py-3 px-4 text-left text-sm font-semibold text-gray-700" data-sort="priority">
                                    Priority <span class="sort-icon"></span>
                                </th>
                                <th class="sortable-header py-3 px-4 text-left text-sm font-semibold text-gray-700" data-sort="status">
                                    Status <span class="sort-icon"></span>
                                </th>
                                <th class="sortable-header py-3 px-4 text-left text-sm font-semibold text-gray-700" data-sort="due-date">
                        ...
                        <tbody id="taskList">
                            @forelse($tasks as $task)
                                ...
                                @endphp
                                <tr class="border-b border-gray-100 hover:bg-gray-50 task-card md:table-row" data-task-id="{{ $task->id }}" data-assignee-type="{{ $assigneeType }}" data-assignee-id="{{ $assigneeId }}">
                                    ...
                                    <td class="py-4 px-4" data-label="Assigned To">
                                        <div class="flex items-center flex-wrap gap-2">
                                            @if($task->assigned_to_team && $task->role)
                                                <span class="bg-gradient-to-r from-blue-50 to-indigo-50 text-blue-700 px-3 py-1 rounded-full text-xs font-medium border border-blue-100">
                                                    <i class="fas fa-users mr-1"></i>
                                                    {{ $task->role->name }}
                                                </span>
                                            @else
                                                @foreach($task->users->take(3) as $user)
                                                    <div class="w-8 h-8 rounded-full bg-gradient-to-r from-indigo-100 to-purple-100 flex items-center justify-center text-indigo-600 text-sm font-semibold">
                                                        {{ strtoupper(substr($user->name, 0, 1)) }}
                                                    </div>
                                                @endforeach
                                                @if($task->users->count() > 3)
                                                    <span class="text-gray-500 text-sm font-medium">+{{ $task->users->count() - 3 }}</span>
                                                @endif
                                            @endif
                                        </div>
                                    </td>
                                    ...
                                    <td class="py-4 px-4" data-label="Status">
                                        <span class="priority-badge status-{{ strtolower(str_replace(' ', '-', $task->status)) }}">
                                            <span class="status-indicator {{ strtolower(str_replace(' ', '-', $task->status)) }}"></span>
                                            {{ $task->status }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="py-4 px-4" data-label="Assigned By">
                                        @if($task->assigner)
                                            <div class="flex items-center">
                                                <div class="w-8 h-8 rounded-full bg-gradient-to-r from-gray-100 to-gray-200 flex items-center justify-center text-gray-600 text-sm font-semibold mr-3">
                                                    {{ strtoupper(substr($task->assigner->name, 0, 1)) }}
                                                </div>
                                                <div>
                                                    <span class="text-sm font-medium text-gray-700">{{ $task->assigner->name }}</span>
                                                    @if($task->assigner->roles->isNotEmpty())
                                                        <div class="text-xs text-gray-500">{{ $task->assigner->roles->pluck('name')->implode(', ') }}</div>
                                                    @endif
                                                </div>
                                            </div>
                                        @else
                                    ...
                                    <td class="py-4 px-4" data-label="Actions">
                                        <div class="flex items-center space-x-2 actions-mobile hidden md:flex">
                                            <button class="edit-task text-indigo-600 hover:text-indigo-800 p-2 hover:bg-indigo-50 rounded-lg" data-task-id="{{ $task->id }}" title="Edit">
                                                <i class="fas fa-edit"></i>
                                            </button>
                                            <button class="view-task text-gray-500 hover:text-gray-700 p-2 hover:bg-gray-100 rounded-lg" data-task-id="{{ $task->id }}" title="View">
                                                <i class="fas fa-eye"></i>
                                            </button>
                                            <button class="delete-task text-red-500 hover:text-red-700 p-2 hover:bg-red-50 rounded-lg" data-task-id="{{ $task->id }}" title="Delete">
            ...
            <div class="modal-header">
                <div class="flex justify-between items-center">
                    <div>
                        ...
                        <p class="text-black-100 text-sm mt-1">Fill in the details below to create and assign a new task</p>
                    </div>
            ...
            <form id="taskForm" class="modal-body" enctype="multipart/form-data">
                            ...
                            <div class="space-y-3">
                                <div class="flex space-x-3 items-center">
                                    <div class="input-with-icon flex-1">
                                        ...
                                        <select id="taskCategory" name="category" class="form-input form-select cursor-pointer">
                                ...
                                <div id="customCategoryContainer" class="hidden">
                                    <div class="space-y-3">
                                        <div class="input-with-icon w-full">
                                            ...
                                            <input type="text" id="customCategoryInput" class="form-input w-full" placeholder="Enter custom category name..." maxlength="50">
                                        </div>
                                        <div class="flex space-x-3">
                                            <button type="button" id="saveCustomCategory" class="px-4 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 flex items-center text-sm justify-center flex-1">
                                                <i class="fas fa-check mr-2"></i>
                                                Save Category
                                            </button>
                                            <button type="button" id="cancelCustomCategory" class="px-4 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 flex items-center justify-center flex-1">
                                                <i class="fas fa-times mr-2"></i>
                                                Cancel
                                            </button>
                                        </div>
                                        <p class="text-xs text-gray-500 mt-2">This custom category will be available for future tasks</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="form-section">
                    ...
                    <div class="input-with-icon">
                        ...
                        <textarea id="taskDescription" name="description" rows="4" class="form-input" placeholder="Describe the task in detail..."></textarea>
                    </div>
                </div>
                    ...
                    <div id="individualAssignment" class="mt-4">
                        ...
                        <div class="mb-4">
                            <div class="input-with-icon">
                                ...
                                <input type="text" id="userSearch" class="form-input search-input" placeholder="Search users by name or role...">
                            </div>
                        </div>
                        ...
                        <div class="user-list-container">
                            <div class="p-3 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
                                ...
                                <div class="flex items-center space-x-2">
                                    <button type="button" id="clearSelectionBtn" class="text-xs text-gray-600 hover:text-gray-800 px-2 py-1 rounded border border-gray-300">
                                        Clear All
                                    </button>
                                    <button type="button" id="selectAllBtn" class="text-xs text-indigo-600 hover:text-indigo-800 px-2 py-1 rounded border border-gray-300">
                                        Select All
                                    </button>
                                </div>
                            </div>
                            <div class="p-3 grid grid-cols-1 md:grid-cols-2 gap-2" id="userList">
                                ...
                                <div class="user-checkbox-card">
                                    <input type="checkbox" name="assigned_users[]" value="{{ $user->id }}" class="user-checkbox hidden" id="user-{{ $user->id }}">
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center space-x-3">
                                            <div class="w-8 h-8 rounded-full bg-gradient-to-r from-blue-100 to-indigo-100 flex items-center justify-center text-indigo-600 font-semibold text-sm">
                                                {{ strtoupper(substr($user->name, 0, 1)) }}
                                            </div>
                                            <div>
                                        ...
                                        <div class="check-indicator w-5 h-5 border-2 border-gray-300 rounded flex items-center justify-center transition-all duration-200">
                    ...
                    <div id="teamAssignment" style="display: none;" class="mt-4">
                        <div class="form-grid form-grid-2 gap-6">
                            <div>
                                ...
                                <div class="input-with-icon">
                                    ...
                                    <select id="assignedRole" name="assigned_role" class="form-input form-select cursor-pointer">
                            ...
                            <div>
                                ...
                                <div class="mt-6">
                                    <button type="button" id="addRoleBtn" class="text-sm text-indigo-600 hover:text-indigo-800 flex items-center font-medium">
                        ...
                        <div id="newRoleInput" class="mt-3 hidden">
                            <div class="flex space-x-2">
                                <input type="text" id="roleName" class="flex-1 form-input" placeholder="Enter new role name">
                ...
                <div class="form-section">
                    ...
                    <div>
                        <div id="dragDropArea" class="drag-drop-area rounded-xl p-8 text-center cursor-pointer transition-all duration-300 hover:border-indigo-400 hover:bg-indigo-50">
                            <div class="w-16 h-16 mx-auto mb-4 rounded-full bg-gradient-to-r from-indigo-100 to-purple-100 flex items-center justify-center text-indigo-600">
                                <i class="fas fa-cloud-upload-alt text-2xl"></i>
                            </div>
                            <p class="text-lg font-semibold text-gray-700 mb-2">Drag & drop files here</p>
                            <p class="text-gray-500">or <span class="text-indigo-600 font-semibold cursor-pointer">browse</span> to upload</p>
        ...
        <div class="delete-modal-content fade-in">
            ...
            <div class="delete-modal-body">
                ...
                <div class="flex space-x-4">
                    ...
                    <button id="confirmDeleteBtn" class="flex-1 btn-primary py-3" style="background: linear-gradient(135deg, #dc2626 0%, #b91c1c 100%);">
